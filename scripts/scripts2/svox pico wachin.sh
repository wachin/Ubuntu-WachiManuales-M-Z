...
## WACHIN INSTALAR ESTAS DEPENDENCIAS PARA QUE FUNCIONE
## (Debes tener también instalado audacious)
...
## Wachín lo copió descargó desde(el 2015-06-24):
...
echo "message:Ahora está activado SVOX Pico lang:$(cat /tmp/svox_selang.txt),\npara cerrar el programa escriba y copie,\nal portapapeles (Ctrl + C) la palabra: quit" >&3

#original: echo "message:Démarrage de SVOX Pico lang:$(cat /tmp/svox_selang.txt),\nPour quitter Pico,\ncopiez dans le presse-papier le mot: quit" >&3

# mi wachín traducción: echo "message:Démarrage de SVOX Pico lang:$(cat /tmp/svox_selang.txt),\nPour quitter Pico,\ncopiez dans le presse-papier le mot: quit" >&3
...
audacious -p test.wav
else
echo r.a.s $(cat /tmp/svox_selang.txt)
fi
if [ "${var}" = "quit" ]; then
pico2wave -l $(cat /tmp/svox_selang.txt) -w test.wav "SVOX Pico"
audacious -p test.wav
...
#original en frances: "echo "message:Arrêt de SVOX Pico..." >&3"
#mi wachín traducción: "echo "message:Cerrando SVOX Pico..." >&3"
echo "message:SVOX Pico..." >&3
exec 3>&-
